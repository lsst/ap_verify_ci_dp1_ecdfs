...
def _export(butler, export_file, template_query):
    ...
    skymaps = set(
        butler.registry.queryDataIds(
            "skymap", datasets=TEMPLATE_NAME, collections=butler.collections.defaults
        )
    )
    ...
    with butler.export(filename=export_file, transfer=None) as contents:
        ...
        # Export templates
        templates = butler.registry.queryDatasets(
            TEMPLATE_NAME, collections=butler.collections.defaults,
            where=template_query, findFirst=True
        )
